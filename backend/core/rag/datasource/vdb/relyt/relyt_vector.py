...
class RelytVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> RelytVector:
        ...
        return RelytVector(
            ...
            config=RelytConfig(
                host=gofy_config.RELYT_HOST or "localhost",
                port=gofy_config.RELYT_PORT,
                user=gofy_config.RELYT_USER or "",
                password=gofy_config.RELYT_PASSWORD or "",
                database=gofy_config.RELYT_DATABASE or "default",
            ),
            group_id=dataset.id,
        )
