...
/**
 * Forum command - Opens Gofy community forum
 */
export const forumCommand: SlashCommandHandler<ForumDeps> = {
  name: 'forum',
  description: 'Open Gofy community forum',
  mode: 'direct',

  // Direct execution function
  execute: () => {
    const url = 'https://forum.gofy.ai'
  ...
  async search(args: string, locale: string = 'en') {
    ...
    return [{
      ...
      data: { command: 'navigation.forum', args: { url: 'https://forum.gofy.ai' } },
    }]
  },
  ...
  register(_deps: ForumDeps) {
    registerCommands({
      'navigation.forum': async (args) => {
        const url = args?.url || 'https://forum.gofy.ai'
        window.open(url, '_blank', 'noopener,noreferrer')
      },
    })
  },
