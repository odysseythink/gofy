...
type ToolProviderController interface {
	...
	*/
	GetTools(user_id, tenant_id string) []*tool.Tool
	...
	/*
		...
		:return: tool
	*/
	GetTool(tool_name string) *tool.Tool
	...
	*/
	ValidateCredentials(credentials map[string]any)

	ProviderType() toolsenumtypes.ToolProviderType
}

type BaseToolProviderController struct {
	Identity          *toolsentities.ToolProviderIdentity               `json:"identity"`
	Tools             []*tool.Tool                                      `json:"tools"`
	CredentialsSchema map[string]*toolsentities.ToolProviderCredentials `json:"credentials_schema"`
}

func (controller *BaseToolProviderController) GetCredentialsSchema() map[string]*toolsentities.ToolProviderCredentials {
	return controller.CredentialsSchema
}
func (controller *BaseToolProviderController) GetParameters(real_controller ToolProviderController, tool_name string) []*toolsentities.ToolParameter {
	tools := real_controller.GetTools("", "")
	if len(tools) == 0 {
		panic(exceptions.NewToolNotFoundError(fmt.Sprintf("tool %s not found", tool_name)))
	}
	var tool *tool.Tool
	for _, t := range tools {
		if t.Identity != nil && t.Identity.Name == tool_name {
			tool = t
			break
		}
	}
...
func (controller *BaseToolProviderController) ValidateCredentialsFormat(credentials map[string]any) {
	credentials_schema := controller.CredentialsSchema
	if credentials_schema == nil {
		return
	}
	credentials_need_to_validate := map[string]*toolsentities.ToolProviderCredentials{}
	...
	for credential_name := range credentials {
		if _, ok := credentials_need_to_validate[credential_name]; !ok {
			if controller.Identity == nil {
				panic(exceptions.NewValueError("identity is not set"))
			}
			panic(exceptions.NewToolProviderCredentialValidationError(fmt.Sprintf("credential %s not found in provider %s", credential_name, controller.Identity.Name)))
		}
		...
		if slices.Contains([]toolsenumtypes.CredentialsType{toolsenumtypes.Credentials_SECRET_INPUT, toolsenumtypes.Credentials_TEXT_INPUT}, credential_schema.Type) {
		...
		} else if credential_schema.Type == toolsenumtypes.Credentials_SELECT {
			if real_val, ok := credentials[credential_name].(string); !ok {
				panic(exceptions.NewToolProviderCredentialValidationError(fmt.Sprintf("credential %s should be string", credential_name)))
			} else {
				if !slices.ContainsFunc(credential_schema.Options, func(option *toolsentities.ToolCredentialsOption) bool {
					return real_val == option.Value
				}) {
					panic(exceptions.NewToolProviderCredentialValidationError(fmt.Sprintf("credential %s should be one of %#v", credential_name, credential_schema.Options)))
				}
			}
		}
	...
	for credential_name := range credentials_need_to_validate {
		...
		if credential_schema.Default != nil {
			...
			if slices.Contains([]toolsenumtypes.CredentialsType{
				toolsenumtypes.Credentials_SECRET_INPUT,
				toolsenumtypes.Credentials_TEXT_INPUT,
				toolsenumtypes.Credentials_SELECT,
