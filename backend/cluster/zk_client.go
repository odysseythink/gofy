...
var (
	...
	ErrClientClosed       = errors.New("zooClient service closed")
...
// ZooService holds the root path, service name and instance identifying data you
...
// IZooClient is a wrapper around a lower level ZooKeeper zooClient implementation.
type IZooClient interface {
	...
	// Stop should properly shutdown the zooClient implementation
	Stop()
...
// the zooClient will use to authenticate itself with.
...
// NewZooClient returns a ZooKeeper zooClient with a connection to the server cluster.
// It will return an error if the server cluster cannot be resolved.
func NewZooClient(servers []string, options ...Option) (IZooClient, error) {
	log.SetFlags(log.Lmicroseconds | log.Lshortfile)
	c := &zooClient{}
	err := c.Init(servers, options)
	if err != nil {
		log.Printf("[E]zooClient init failed:%v\n", err)
		return nil, err
	}
	return c, nil
}

func (c *zooClient) Init(args ...interface{}) error {
	if len(args) != 2 {
	...
	if servers, ok := args[0].([]string); !ok || servers == nil {
		...
		if options, ok := args[1].([]Option); !ok {
			...
			var err error
			c.zkConn, c.eventc, err = zkConnect(servers, c.clientConfig.sessionTimeout, zkWithLogger(nil), zkWithDialer(dialer))
...
func (c *zooClient) CreateParentNodes(path string) error {
	if c.zkConn == nil {
...
func (c *zooClient) GetEntries(path string) ([]string, <-chan zkEvent, error) {
...
func (c *zooClient) GetNodesAndEntries(path string) (map[string]string, <-chan zkEvent, error) {
	// retrieve list of child nodes for given path and add watch to path
	znodes, _, eventc, err := c.ChildrenW(path)
...
func (c *zooClient) GetWNodesAndEntries(path string) (map[string]string, map[string]<-chan zkEvent, <-chan zkEvent, error) {
	// retrieve list of child nodes for given path and add watch to path
	znodes, _, eventc, err := c.ChildrenW(path)
...
func (c *zooClient) GetWEntry(path string) (string, <-chan zkEvent, error) {
	data, _, eventc, err := c.GetW(path)
	return string(data), eventc, err
}

// Register implements the ZooKeeper Client interface.
func (c *zooClient) Register(path, name, data string) (err error) {
	...
	c.services.Range(func(key, value any) bool {
		...
		if s, ok := value.(*ZooService); !ok && s == nil {
...
// Register implements the ZooKeeper Client interface.
func (c *zooClient) RegisterDirect(path, name, data string) error {
	if path[len(path)-1] != '/' {
		path += "/"
	}
	fullpath := path + name
...
func (c *zooClient) Deregister(path, name string) error {
	if path[len(path)-1] != '/' {
		path += "/"
	}
	fullpath := path + name
	...
	if val, ok := c.services.Load(fullpath); !ok {
		...
		if s, ok := val.(*ZooService); !ok && s == nil {
...
func (c *zooClient) DeleteFullpath(fullpath string) error {
...
func (c *zooClient) AddInstancer(path string, cb InstancerCallBack, errcb InstancerErrCallBack, childcb ChildCallBack) error {
...
func (c *zooClient) StopInstancer(path string) error {
	if _, ok := c.instanceres[path]; !ok {
...
func (w *clientEventWatcher) Init(args ...interface{}) error {
	...
	if cli, ok := args[0].(*zooClient); !ok || cli == nil {
		log.Printf("[E]args[0] must be zooClient pointer")
		return errors.New("args[0] must be zooClient pointer")
	} else {
		w.cli = cli
		if w.cli.eventc == nil {
			log.Printf("[E]zooClient init failed, not have eventc")
			return errors.New("zooClient init failed, not have eventc")
		}
		return nil
	}
}
