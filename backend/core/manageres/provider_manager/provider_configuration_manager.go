...
func (pm *ProviderConfigurationManager) GetModels(
	pcs *coreentities.ProviderConfigurations, provider string, model_type modelruntimeenumtypes.ModelType, only_active bool,
) []*modelentities.ModelWithProviderEntity {
	/*
	...
	*/
	...
	for _, provider_configuration := range pcs.Configurations {
		...
		all_models = append(all_models, pm.GetProviderModels(provider_configuration, model_type, only_active)...)
	}
	return all_models
}
