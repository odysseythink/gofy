...
func newClient(cfg *Config) (*Client, error) {
	...
	go client.autoSync()
	_, err = client.Get(client.Ctx(), "/etcd-"+strings.ReplaceAll(strings.ReplaceAll(strings.ReplaceAll(time.Now().Format("2006-01-02 15:04:05.000000"), " ", ""), ":", ""), ".", ""))
	if err != nil {
		client.Close()
		return nil, err
	}
...
func minSupportedVersion() *version.SemanticVersion {
	ver := version.MustSemanticVersion(version.NewSemanticVersionE(version.Version))
	// consider only major and minor version
	ver = &version.SemanticVersion{Major: ver.Major, Minor: ver.Minor}
...
func (c *Client) checkVersion() (err error) {
	...
	for _, ep := range eps {
		...
		go func(e string) {
			...
			vs, serr := version.NewSemanticVersionE(resp.Version)
			if serr != nil {
				errc <- serr
				return
			}
