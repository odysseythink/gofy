...
func (p PluginUniqueIdentifier) Version() manifestentities.Version {
	// extract version part from the string
	split := strings.Split(p.String(), "@")
	if len(split) == 2 {
		split = strings.Split(split[0], ":")
		if len(split) == 2 {
			return manifestentities.Version(split[1])
		}
	}
	return ""
}
...
func (p PluginUniqueIdentifier) RemoteLike() bool {
	...
	_, err := uuid.FromString(p.Author())
	return err == nil
}
...
func (p PluginUniqueIdentifier) Validate() error {
	return validate.GlobalEntitiesValidator.Var(p, "plugin_unique_identifier")
}
...
func init() {
	validate.GlobalEntitiesValidator.RegisterValidation("plugin_unique_identifier", isValidPluginUniqueIdentifier)
}
