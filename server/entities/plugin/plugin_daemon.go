...
type PluginDaemonBasicResponse(BaseModel, Generic[T]):
    code: int
    message: str
    data: Optional[T]
}
type InstallPluginMessage(BaseModel):
    class Event(StrEnum):
        Info = "info"
        Done = "done"
        Error = "error"
...
type PluginToolProviderEntity(BaseModel):
    provider: str
    plugin_unique_identifier: str
    plugin_id: str
    declaration: ToolProviderEntityWithPlugin
}
type PluginAgentProviderEntity(BaseModel):
    provider: str
    plugin_unique_identifier: str
    plugin_id: str
...
type PluginBasicBooleanResponse(BaseModel):
    result: bool
    credentials: dict | None = None
}
type PluginModelSchemaEntity(BaseModel):
    model_schema: AIModelEntity = Field(description="The model schema.")

    # pydantic configs
    model_config = ConfigDict(protected_namespaces=())
}
type PluginModelProviderEntity(BaseModel):
...
type PluginInstallTaskStatus(StrEnum):
    Pending = "pending"
    Running = "running"
    Success = "success"
    Failed = "failed"
}
type PluginInstallTaskPluginStatus(BaseModel):
...
type PluginInstallTask(BasePluginEntity):
    status: PluginInstallTaskStatus = Field(description="The status of the install task.")
...
type PluginInstallTaskStartResponse(BaseModel):
...
type PluginVerification(BaseModel):
    """
...
type PluginDecodeResponse(BaseModel):
...
type PluginOAuthAuthorizationUrlResponse(BaseModel):
    authorization_url: str = Field(description="The URL of the authorization.")
}
type PluginOAuthCredentialsResponse(BaseModel):
...
type PluginListResponse(BaseModel):
    list: list[PluginEntity]
    total: int
}
type PluginDynamicSelectOptionsResponse(BaseModel):
