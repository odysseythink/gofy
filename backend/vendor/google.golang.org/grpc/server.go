...
// RegisterServiceDesc is called from generated code and maps from the fully qualified
// proto name to the type (pointer to struct) of the protocol buffer.
func RegisterServiceDesc(x *ServiceDesc) {
	if x == nil {
		log.Printf("grpc: ServiceDesc must be provided")
		return
	}
	fmt.Println("----------x.ServiceName=", x.ServiceName)
	if _, ok := grpcServiceDescs[x.ServiceName]; ok {
		// TODO: Some day, make this a panic.
		log.Printf("grpc: duplicate service type registered: %v", x.ServiceName)
		return
	}

	grpcServiceDescs[x.ServiceName] = x
}

var (
	statusOK         = status.New(codes.OK, "")
	logger           = grpclog.Component("core")
	grpcServiceDescs = make(map[string]*ServiceDesc)
)
...
func (s *Server) RegisterServiceWithoutDesc(ss interface{}, servicename string) error {
	var sd *ServiceDesc = nil
	if ss != nil {
		for k, v := range grpcServiceDescs {
			if k == servicename {
				ht := reflect.TypeOf(v.HandlerType).Elem()
				st := reflect.TypeOf(ss)
				if st.Implements(ht) {
					logger.Infof("grpc: Server.RegisterServiceWithoutDesc found the handler of type %v", st)
					sd = v
					break
				} else {
					logger.Errorf("service(%#v) not implement grpc service=%#v", ss, v.HandlerType)
				}
			}
		}
	}
	if sd == nil {
		logger.Errorf("can't find ServiceDesc By service(%#v)", reflect.TypeOf(ss))
		return fmt.Errorf("can't find ServiceDesc By service(%#v)", reflect.TypeOf(ss))
	}
	s.register(sd, ss)
	return nil
}
