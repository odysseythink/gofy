...
describe('useDocLink', () => {
  ...
  describe('Base URL handling', () => {
    ...
    it('should remove trailing slash from base URL', () => {
      const baseUrlWithSlash = 'https://docs.gofy.ai/'
      const { result } = renderHook(() => useDocLink(baseUrlWithSlash))
      const url = result.current('/use-gofy/getting-started/introduction')
      expect(url).toBe('https://docs.gofy.ai/en/use-gofy/getting-started/introduction')
    })

    it('should handle base URL without trailing slash', () => {
      const baseUrlWithoutSlash = 'https://docs.gofy.ai'
  ...
  describe('Path handling', () => {
    it('should handle path parameter', () => {
      const { result } = renderHook(() => useDocLink())
      const url = result.current('/use-gofy/getting-started/introduction')
      expect(url).toBe(`${defaultDocBaseUrl}/en/use-gofy/getting-started/introduction`)
    })
  ...
  describe('PathMap handling', () => {
    it('should use path from pathMap when locale matches', () => {
      ...
      const pathMap: DocPathMap = {
        'zh-Hans': '/use-gofy/getting-started/introduction',
        'en-US': '/use-gofy/getting-started/quick-start',
      }

      const { result } = renderHook(() => useDocLink())
      const url = result.current('/use-gofy/getting-started/quick-start' as DocPathWithoutLang, pathMap)
      expect(url).toBe(`${defaultDocBaseUrl}/zh/use-gofy/getting-started/introduction`)
    })
    ...
    it('should use default path when locale not in pathMap', () => {
      ...
      const { result } = renderHook(() => useDocLink())
      const url = result.current('/use-gofy/getting-started/quick-start' as DocPathWithoutLang, pathMap)
      expect(url).toBe(`${defaultDocBaseUrl}/ja/use-gofy/getting-started/quick-start`)
    })

    it('should handle undefined pathMap', () => {
      const { result } = renderHook(() => useDocLink())
      const url = result.current('/use-gofy/getting-started/introduction', undefined)
      expect(url).toBe(`${defaultDocBaseUrl}/en/use-gofy/getting-started/introduction`)
    })
  })
  ...
  describe('Edge Cases', () => {
    it('should handle path with anchor', () => {
      const { result } = renderHook(() => useDocLink())
      const url = result.current('/use-gofy/getting-started/introduction#overview' as DocPathWithoutLang)
      expect(url).toBe(`${defaultDocBaseUrl}/en/use-gofy/getting-started/introduction#overview`)
    })

    it('should handle multiple calls with same hook instance', () => {
      const { result } = renderHook(() => useDocLink())
      const url1 = result.current('/use-gofy/getting-started/introduction')
      const url2 = result.current('/use-gofy/getting-started/quick-start')
      expect(url1).toBe(`${defaultDocBaseUrl}/en/use-gofy/getting-started/introduction`)
      expect(url2).toBe(`${defaultDocBaseUrl}/en/use-gofy/getting-started/quick-start`)
    })
  })
})
